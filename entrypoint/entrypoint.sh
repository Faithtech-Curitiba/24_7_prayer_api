...
display_help() {
    ...
    echo "   -c, --cron                 Start the server in cron mode"
...
start_server_with_newrelic(){
    ...
    NEW_RELIC_CONFIG_FILE=newrelic.ini newrelic-admin run-program gunicorn -b $S_ADDRESS:$PORT -w 3 --forwarded-allow-ips="*"  prayer.wsgi
    exit_with_error
}

start_server(){
    echo "Starting gunicorn..."
    gunicorn -b $S_ADDRESS:$PORT -w 3 --forwarded-allow-ips="*"  prayer.wsgi
}
...
start_server_as_cron(){
    echo "Starting server in cron mode..."
    cron
    python manage.py crontab add
}

create_cron_env_file(){
    printenv | sed 's/^\(.*\)$/export "\1"/g' > /app/cron_env.sh
}
...
choose_starting_way(){
    ...
    if [ $PCT -eq 1 ]; then create_cron_env_file && start_server_as_cron; fi
